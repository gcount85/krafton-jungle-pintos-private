...
/*########################Newly added in Project 2#########################*/
/*--------------Process System Call----------------*/
/* Find child process with 'child_tid' in current thread's child_list list
...
find_child_process (tid_t child_tid) {
	...
	/* Walking through the child_list list */
	for (child = list_begin(&curr->child_list); 
		 child != list_end (&curr->child_list); child = list_next (child)) {
...
init_thread (struct thread *t, const char *name, int priority) {
	...
	/*###############Newly added in Project 2###############*/
	/*-----------------Process System Call------------------*/
	list_init (&t->child_list); /* Initialize a process' child_list list*/
	sema_init (&t->sema_for_wait, 0); /*Initialize semaphore for wait(), exit()*/
